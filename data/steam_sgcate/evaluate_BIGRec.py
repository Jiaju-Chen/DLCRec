...
for p in path:
    ...
    for i, batch_input in tqdm(enumerate(text)):
        items = predict_item_pattern.findall(batch_input)
        if len(items)==0: items = batch_input.replace("\"", "").split('| ')[:10]
        if len(items) != 10:
            print(f"output movie number error! {i} {len(items)} {items}")
    ...
    for topk in topk_list:
        ...
        for i in range(len(test_data)):
            ...
            if match:
                ...
                match = re.search(r'with (\w+) genres', text)
                if match:
                    target_genre_num = int(match.group(1))
                else:
                    target_genre_num = 0
            ...
            target_movies = predict_item_pattern.findall(test_data[i]['output']) # test_data[i]['output'].replace("\"", "").split('| ')[:10]
...
f = open('./result_BIGRec_steam.json', 'w')
