...
for p in path:
    ...
    for i, batch_input in tqdm(enumerate(text)):
        items = batch_input.replace("\"", "").split('| ')[:10]
        if len(items) != 10:
    ...
    movie_embedding = torch.load("./data/movie/movie_embedding_llama3.pt").cuda()
    ...
    for topk in topk_list:
        ...
        for i in range(len(test_data)):
            ...
            target_movies = test_data[i]['output'].replace("\"", "").split('| ')[:10]
...
f = open('./result_BIGRec_movie.json', 'w')
